...
Promises.chrome.tabGroups = class {
  ...
  */
  ...
  /**
   * A promise wrapper for chrome.tabGroups.get()
   * see: https://developer.chrome.com/docs/extensions/reference/tabGroups/#method-get
   * @param {number} groupId the group's id
   * @returns {Promise<TabGroup>} a TabGroup
  */
  static async get(groupId) {
    return new Promise((resolve, reject) => {
      try {
        chrome.tabGroups.get(groupId, tabGroup => resolve(tabGroup))
      } catch (e) {
        reject(e)
      }
    })
